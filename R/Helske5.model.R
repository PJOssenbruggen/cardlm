#' \code{Helske5.model} is a wrapper function ring road experiment
#'
...
# #' Helske5.model(usd)
#' @export
Helske5.model <- function(usd, zsd) {
  set.seed(123)
  start  <- 0
  end    <- 40
  tseq   <- seq(0,40,0.125)
  ...
  ts.plot(window(data, start = c(0,1), end = c(15,0))[,c(7,8)],
          col = c("black", "blue"),
  ...
  ########################################################################################
  model  <- SSModel(data[,7] ~  -1 + SSMcycle(period = 2*pi, Q = NA),
                    H = matrix(NA,1,1),
  ...
  ts.plot(window(Out, start = c(0,1), end = c(15,0))[,c(3,5,11,12)],
